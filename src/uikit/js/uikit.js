...
(function (global, factory) {
    ...
    for (const position of top ? [0, top] : [0]) {
      ...
      for (const el of toWindow(target).document.elementsFromPoint(
        ...
        if (
          ...
          (!coverEl || dimensions$1(coverEl).height < dimensions$1(el).height)
        ) {
          coverEl = el;
        }
      }
  ...
  function fade(action, target, duration, stagger = 0) {
    ...
    const wrapIndexFn = (fn) => () =>
      isCurrentIndex() ? fn() : Promise.reject();
    ...
    const enterFn = wrapIndexFn(async () => {
      ...
      if (!stagger || oldHeight !== newHeight) {
        ...
        transitions.push(
          Transition.start(target, targetProps, targetDuration, 'ease')
        );
      }
            ...
            toNodes(targets).map((el) => {
              ...
              const done = () => {
                ...
                if (show) {
                  ...
                  (_a = $$('[autofocus]', el).find(isVisible)) == null
                    ? void 0
                    : _a.focus();
                  restoreScrollPosition();
                }
  ...
  var SliderDrag = {
    ...
    methods: {
      ...
      move(e) {
        ...
        if (this.dragging) {
          setTimeout(
            on(this.list, 'click', (e) => e.preventDefault(), pointerOptions)
          );
          this.dragging = null;
    ...
    methods: {
      ...
      padNavitems() {
        ...
        for (let i = 0; i < this.length; i++) {
          ...
          children2[i] =
            this.navChildren.findLast((el) => el.matches(`[${attr2}]`)) ||
  ...
  var LightboxPanel = {
    ...
    created() {
      ...
      remove$1(
        $$(this.selNav, $el).filter((el) => !matches(el, `.uk-${navType}`))
      );
      for (const [i, item] of this.items.entries()) {
        ...
        if (navType === 'thumbnav') {
          wrapAll(
            ...
            append(
              $(this.selNav, $el),
              `<li uk-lightbox-item="${i}"><a href></a></li>`
            )
          );
        }
      }
      ...
      },
      {
        ...
        async handler(_, item) {
          ...
          if (type === 'image' || isImage(src)) {
            ...
            (matches2 = src.match(/\/\/.*?vimeo\.[a-z]+\/(\d+)[&?]?(.*)?/))
          ) {
            try {
  ...
  function toThumbnavItem(item, videoAutoplay) {
    const el =
      item.poster ||
      (item.thumb && (item.type === 'image' || isImage(item.thumb)))
  ...
  function ensureThumb(toggles, items) {
    for (const [i, toggle] of Object.entries(toggles)) {
      ...
      const parent = parents(toggle)
        ...
        .find(
          (parent2) =>
            this.$el.contains(parent2) &&
            (parent2 === toggle || $$(this.toggle, parent2).length === 1)
        );
      if (!parent) {
        continue;
      }
  ...
  function Transitioner(prev, next, dir, { center, easing, list }) {
    ...
    return {
      ...
      percent() {
        return Math.abs(
          (new DOMMatrix(css(list, 'transform')).m41 * (isRtl ? -1 : 1) +
            from) /
            (to - from)
        );
      },
      ...
      once(
        ...
        ({ defaultPrevented, type, target: newTarget }) => {
          ...
          if (
            ...
            type === pointerUp$1 &&
            target === newTarget &&
            !((_a = drop.targetEl) == null ? void 0 : _a.contains(target))
          ) {
            drop.hide(false);
          }
        },
          ...
          if (keyCode === keyMap.DOWN) {
            if ((active2 == null ? void 0 : active2.targetEl) === current) {
              ...
              const dropdown = this.dropdowns.find((el) => {
                var _a2;
                return (
                  ((_a2 = this.getDropdown(el)) == null
                    ? void 0
                    : _a2.targetEl) === current
                );
              });
              if (dropdown) {
                e.preventDefault();
  ...
  function install({ modal }) {
    ...
    modal.alert = function (message, options) {
      return openDialog(
        ({ i18n }) =>
          `<div class="uk-modal-body">${isString(message) ? message : html(message)}</div> <div class="uk-modal-footer uk-text-right"> <button class="uk-button uk-button-primary uk-modal-close" type="button" autofocus>${i18n.ok}</button> </div>`,
        options
      );
    };
    ...
    events: [
      ...
      },
      {
        ...
        async handler() {
          ...
          if (
            this._transparent &&
            (!active || !this.dropContainer.contains(active.$el))
          ) {
      ...
      resize({
        ...
        handler(entries) {
          this.$emit(
            this._data.resized &&
              entries.some(
                ({ target }) => target === getVisibleParent(this.$el)
              )
              ? 'update'
              : 'resize'
          );
    ...
    update: [
      {
        read({ height: height$1, width, margin, sticky }, types) {
          this.inactive =
            !this.matchMedia || !isVisible(this.$el) || !this.$el.offsetHeight;
          if (this.inactive) {
            return;
          }
          ...
          const overflow = this.overflowFlip
            ? 0
            : Math.max(0, elementBox - viewport2);
          const topOffset =
